...
TEST_DOLLAR_PHONE_CORPUS_PATH = path.join(
    DATA_DIR, 'test_dollar_phone_corpus.txt')
...
ALL_OANC_DOLLAR_PROGRAM_OUT_PATH = path.join(
    OUT_DIR, 'all-OANC_dollar_program.txt')
ALL_OANC_TELEPHONE_REGEXP_OUT_PATH = path.join(
    OUT_DIR, 'all-OANC_telephone_regexp.txt')
TEST_DOLLAR_PROGRAM_OUT_PATH = path.join(OUT_DIR, 'test_dollar_program.txt')
TEST_TELEPHONE_REGEXP_OUT_PATH = path.join(
    OUT_DIR, 'test_telephone_regexp.txt')
...
def get_output_statistics(output_path):
    ...
    output_counter = Counter([line.strip()
...
def evaluate_outputs(reference_path, out_path):
    ...
    reference_counter = Counter(
        [line.strip() for line in reference_lines if line.strip() != ''])
    out_counter = Counter([line.strip()
                          for line in out_lines if line.strip() != ''])
